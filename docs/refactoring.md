...
### Tier 3 — Средний приоритет ✅ Выполнено

- [~] **Пакетная структура `app/`** — *отклонено: проект уже организован (database/, docs/, templates/), перемещение 7 файлов даёт глубину без реальной пользы при данном масштабе*
- [~] **Нормализовать схему trades** — *отклонено: 3 DCA — фиксированное бизнес-правило стратегии, нормализация добавляет JOIN-сложность без пользы*
- [x] **Типизация** — `Any` заменён на `"Notifier"`, `"BybitExchange"`, `"TelegramClient"`, `aiosqlite.Row` через `TYPE_CHECKING`
- [x] **Реестр команд** в `notifier.py` — команды разбиты на методы `_cmd_*`, диспетчер через `self._commands` dict
- [x] **Устранить N+1 запросы** в `web_server.py` — `get_open_trades()` вызывается один раз, lookup по dict

### Tier 4 — Низкий приоритет (качество и polish)

- [x] **Пиннинг зависимостей** в `requirements.txt` — добавлены точные версии всех 9 пакетов
- [~] **Тесты** в `tests/` — *отклонено: проект уже в Prod, тесты не дают практической пользы*
- [x] **Валидация конфига** при старте — `_validate()` проверяет TG_API_HASH, BYBIT_API_KEY, BYBIT_API_SECRET, DEPO_USDT > 0
- [~] **Улучшить `parser.py`** — *отклонено: парсер работает надёжно в Prod*
- [~] **Метки времени** в БД — *отклонено: ISO-строки сортируются корректно, миграция несёт риск без реальной пользы*
